...
const AssetMatrix = dynamic(() => import('@/components/global/AssetMatrix'), { ssr: false });
const MobilePodGallery = dynamic(() => import('@/components/global/MobilePodGallery'), { ssr: false });
const NeuralStudioApp = dynamic(() => import('@/components/global/NeuralStudioApp'), { ssr: false });
const MatrixSidebar = dynamic(() => import('@/components/neural/MatrixSidebar'));

export default function PrivateMatrix() {
    const { currentUser, isArchitect } = useUserStore();
    const router = useRouter();
    const [isSwipeDeckView, setIsSwipeDeckView] = useState(false); // Default false for SSR, will sync on mount
    ...
    useEffect(() => {
        ...
        const handleResize = () => {
            if (window.innerWidth >= 768) {
                setIsSwipeDeckView(false); // Desktop defaults to gallery
            } else {
                setIsSwipeDeckView(true); // Mobile defaults to swipe deck
            }
        };
