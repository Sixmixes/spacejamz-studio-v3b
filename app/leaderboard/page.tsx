                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col gap-3 relative z-10">
                        {leaders.map((pilot, idx) => (
                            <div 
                                key={pilot.uid} 
                                className={`group relative flex items-center gap-4 p-4 md:p-6 border transition-all duration-500 overflow-hidden ${
                                    idx === 0 ? 'border-yellow-500/50 shadow-[0_0_40px_rgba(234,179,8,0.2)] scale-[1.02]' :
                                    idx === 1 ? 'border-zinc-400/30' :
                            ...
                            >
                                ...
                                <div className="absolute inset-0 z-0 pointer-events-none">
                                    {pilot.customBannerUrl ? (
                                        <div className="w-full h-full relative overflow-hidden">
                                            <img 
                                                src={pilot.customBannerUrl} 
                                                className="w-full h-full object-cover opacity-30 grayscale-[0.5] group-hover:grayscale-0 group-hover:scale-110 group-hover:opacity-60 transition-all duration-[3s]"
                                                style={{ 
                                                    objectPosition: `50% ${pilot.bannerPositionY ?? 50}%`,
                                                    transform: `scale(${pilot.bannerZoom ?? 1})`
                                                }}
                                            />
                                            <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-transparent" />
                                        </div>
                                    ) : (
                                        <div className="w-full h-full bg-[#050505] group-hover:bg-zinc-900 transition-colors duration-500" />
                                    )}
                                </div>

                                <div className="relative z-10 flex items-center gap-4 md:gap-6 w-full">
                                    <div className="flex items-center justify-center w-8 shrink-0 font-bebas text-2xl md:text-4xl text-white/40 tracking-widest group-hover:text-white transition-colors">
                                        {idx === 0 ? <Crown className="text-yellow-500 drop-shadow-[0_0_12px_rgba(234,179,8,0.8)]" size={idx === 0 ? 32 : 24} /> : `#${idx + 1}`}
                                    </div>
    
                                    <div className="w-12 h-12 md:w-20 md:h-20 shrink-0 bg-black border-2 border-white/20 p-0.5 shadow-2xl relative" style={{ clipPath: 'polygon(0 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%)' }}>
                                        <img 
                                    ...
                                    <div className="flex-1 min-w-0 pr-4">
                                        <h3 className={`text-2xl md:text-3xl font-bebas tracking-widest uppercase truncate leading-none ${idx === 0 ? 'text-yellow-400 font-bold drop-shadow-[0_0_10px_rgba(234,179,8,0.4)]' : 'text-white'}`}>
                                            {pilot.displayName || 'UNREGISTERED PILOT'}
                                        </h3>
                                        <div className="flex items-center gap-4 mt-2">
                                            <span className="font-mono text-[8px] md:text-[10px] text-gray-500 uppercase tracking-[0.4em] font-black group-hover:text-gray-300 transition-colors">
                                                RANK: {Math.floor((pilot.xp || 0) / 1000) + 1}
                                            </span>
                                            {pilot.role === 'FOUNDER' && (
                                                <span className="font-mono text-[8px] md:text-[9px] bg-yellow-500/20 text-yellow-500 px-2 py-0.5 border border-yellow-500/30 uppercase tracking-[0.3em] font-black animate-pulse">Founder</span>
                                            )}
                                        </div>
                                    </div>
    
                                    <div className="flex flex-col items-end gap-1 shrink-0">
                                        <div className="flex items-center gap-2 text-primary bg-primary/10 px-4 py-2 border-2 border-primary/20 shadow-[0_0_20px_rgba(var(--color-primary),0.1)] group-hover:border-primary/60 transition-all">
                                            <Zap size={14} className="animate-pulse" />
                                            <span className="font-mono text-[10px] md:text-lg font-black tracking-[0.2em] uppercase">{pilot.xp?.toLocaleString() || 0} XP</span>
                                        </div>
                                        <span className="font-mono text-[7px] text-primary/40 uppercase tracking-[0.5em] font-black hidden md:block">Tactical_XP_Balance</span>
                                    </div>
                                </div>
                            </div>
                        ))}
